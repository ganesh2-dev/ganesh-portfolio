...
@WebServlet("/EditProv")
public class EditProv extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      String user= (String) getServletContext().getAttribute("user");
        ...
        try {
            ...
            String sql = "SELECT * FROM provdetails WHERE provid=?";
            PreparedStatement pstmt = con.prepareStatement(sql);
            pstmt.setString(1, user);
            ResultSet rs = pstmt.executeQuery();

            out.println("<div class='container'>");
            out.println("<form action='UpdateProv' method='POST'>");
            out.println("<table>");

            if (rs.next()) {
                String cust_id = rs.getString("provid");
